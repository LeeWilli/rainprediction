...
ALGOS_TO_RUN = ('knn', 'svm') # lasso, xgboost, knn, stepwise
TRAIN_RECIPE_PATH = 'data/recipe.p'
OUTPUT_PATH = 'data/output.p'
STATIC_RECIPE_PATH = 'data/static.p'
...
def train(X, Y):
    ...
    for kf_id, (train_indices, valid_indices) in enumerate(kfold.split(X)):
        ...
        # PCA
        if 'xgboost' in ALGOS_TO_RUN or 'lasso' or 'svm' in ALGOS_TO_RUN:
    ...
    return algos[int(np.argmin(algos_scores))], algos_scores[int(np.argmin(algos_scores))]


def predict(X, Y, test_X, best_algo):
    # PCA
    pred_test_Ys = []
    if 'xgboost' in best_algo or 'lasso' or 'svm' in best_algo:
        pca = PCA(n_components=40)
        pca.fit(X)
        print("pca explained variance ratio: {}...".format(sum(pca.explained_variance_ratio_)))
...
if __name__ == "__main__":
    ...
    create_pickle(recipe_count, STATIC_RECIPE_PATH)
    print(recipe_count)
    # predict
    output = {}
    # TODO: load recipe
